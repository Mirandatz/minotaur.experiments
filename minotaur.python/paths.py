...
if _PC_NAME == 'Tchon':
    ...
    _MINOTAUR_PATH = _base_dir / 'minotaur' / 'minotaur' / 'minotaur' / 'bin' / 'x64' / 'release' / 'netcoreapp3.1'
    ...
    _MINOTAUR_PATH = _MINOTAUR_PATH / 'netcoreapp3.1' / 'Minotaur'
else:
...
def get_datasets_paths(dataset_name: str, fold_nr: int) -> Dict[str, Path]:
    dataset_dir = _DATASETS_BASE_DIR / dataset_name / 'folds'
    return {'train_data': dataset_dir / str(fold_nr) / 'train-data.csv',
            'train_labels': dataset_dir / str(fold_nr) / 'train-labels.csv',
            'test_data': dataset_dir / str(fold_nr) / 'test-data.csv',
            'test_labels': dataset_dir / str(fold_nr) / 'test-labels.csv'}
